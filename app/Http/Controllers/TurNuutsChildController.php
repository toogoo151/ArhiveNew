...
class TurNuutsChildController extends Controller
{
    ...
    {
        try {
            $turNuutsChild = TurNuutsChild::where("hnID", "=", $req->_parentID)
                ->orderBy('id', 'desc')
                // $TurNuutsChild = DB::table("db_arhivturnuuts")
                // ->where("way_parent", "=", $req->_parentID)
                ->get();
            return $turNuutsChild;
        } catch (\Throwable $th) {
            // throw $th;
        }
    }
    ...
    public function NewChildTurNuuts(Request $req)
    {
        ...
        try {
            ...
            $insertTur->barimt_dugaar = $req->barimt_dugaar; // integer эсвэл validation шалгах
