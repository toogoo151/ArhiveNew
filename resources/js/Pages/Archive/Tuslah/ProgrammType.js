...
const ProgrammType = () => {
    ...
    const [getProgrammType, setProgrammType] = useState([]);
    ...
    const importExcel = (file) => {
        ...
        axios
            .post("/import/SecretTypeTuslah", formData)
            .then((res) => {
                Swal.fire(res.data.msg); // Мэдэгдэл
                refreshProgrammType(); // <-- Table refresh хийж өгөгдөл шинэчлэгдэх
            })
            .catch((err) => {
    ...
    const refreshProgrammType = () => {
        axios
            .get("/get/SecretTypeTuslah")
            .then((res) => {
                setRowsSelected([]);
                setallHumrug(res.data);
                setProgrammType(res.data); // анх бүх өгөгдөл
    ...
    useEffect(() => {
        if (getRowsSelected[0] !== undefined) {
            ...
            setclickedRowData(getProgrammType[getRowsSelected[0]]);
        }
    }, [getRowsSelected, getProgrammType]);

    //  DATE FILTER
    useEffect(() => {
        if (!isFilterActive) {
            setProgrammType(allHumrug);
            return;
        }
    ...
    const btnDelete = () => {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios
                    .post("/delete/humrug", {
                        id: getProgrammType[getRowsSelected[0]].id,
                    })
                    .then((res) => {
            ...
            <div className="row">
                <div className="info-box">
                    <div className="col-md-12">
                        <h1 className="text-center mb-4">Programm Type</h1>

                        {/* TABLE */}
                        <div className="col-md-12 mb-3">
                            <label
                                htmlFor="humrugExcelProgrammType"
                        ...
                        <MUIDatatable
                            data={getProgrammType}
                            setdata={setProgrammType}
                            columns={columns}
                            costumToolbar={
                                <CustomToolbar
                                    ...
                                    buttonName="Нэмэх"
                                    excelDownloadData={getProgrammType}
