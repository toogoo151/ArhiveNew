...
import TurNuutsPrint from "./TurNuutsPrint";

const TurNuutsShiljuuleh = ({
    selectedHumrug,
    selectedDans,
    changeDataRow,
    getTurNuuts,
    ...
    const [clickedRowData, setclickedRowData] = useState(
        getRowsSelected[0] !== undefined
            ? getTurNuuts[getRowsSelected[0]]
            : null
    );
    const rowData =
        getRowsSelected.length > 0
            ? getTurNuuts[getRowsSelected[0]]
            : getTurNuuts[0]; // эхний мөр автоматаар
    useEffect(() => {
        if (getRowsSelected.length > 0) {
            const selectedData = getRowsSelected.map(
                (rowIndex) => getTurNuuts[rowIndex]
            );
    ...
    const removeSelectedRow = (rowId) => {
        ...
        const updatedIndexes = updatedRows.map((row) =>
            getTurNuuts.findIndex((r) => r.id === row.id)
        );
...
export default TurNuutsShiljuuleh;
const columns = [
    {
        name: "id",
        label: "№",
