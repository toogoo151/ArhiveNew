...
const BaingaIltShiljuuleh = ({
    ...
    const [clickedRowData, setclickedRowData] = useState(
        getRowsSelected[0] !== undefined
            ? getBaingaIlt[getRowsSelected[0]]
            : null
    );
    const rowData =
        getRowsSelected.length > 0
            ? getBaingaIlt[getRowsSelected[0]]
            : getBaingaIlt[0]; // эхний мөр автоматаар
    useEffect(() => {
        if (getRowsSelected.length > 0) {
            const selectedData = getRowsSelected.map(
                (rowIndex) => getBaingaIlt[rowIndex]
            );
    ...
    const removeSelectedRow = (rowId) => {
        ...
        const updatedIndexes = updatedRows.map((row) =>
            getBaingaIlt.findIndex((r) => r.id === row.id)
        );
...
export default BaingaIltShiljuuleh;
const columns = [
    {
        name: "id",
        label: "№",
