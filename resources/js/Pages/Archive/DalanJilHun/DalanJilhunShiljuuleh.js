...
import DalanJIlhunPrint from "./DalanJIlhunPrint";

const DalanJilhunShiljuuleh = ({
    selectedHumrug,
    selectedDans,
    changeDataRow,
    getDalHun,
    ...
    const [clickedRowData, setclickedRowData] = useState(
        getRowsSelected[0] !== undefined ? getDalHun[getRowsSelected[0]] : null
    );
    const rowData =
        getRowsSelected.length > 0
            ? getDalHun[getRowsSelected[0]]
            : getDalHun[0]; // эхний мөр автоматаар
    useEffect(() => {
        if (getRowsSelected.length > 0) {
            const selectedData = getRowsSelected.map(
                (rowIndex) => getDalHun[rowIndex]
            );
    ...
    useEffect(() => {
        if (getRowsSelected.length > 0) {
            const rows = getRowsSelected.map((rowIndex) => getDalHun[rowIndex]);
    ...
    const removeSelectedRow = (rowId) => {
        ...
        const updatedIndexes = updatedRows.map((row) =>
            getDalHun.findIndex((r) => r.id === row.id)
        );
                    ...
                    <div className="modal-footer">
                        <button
                            ...
                            onClick={() => {
                                ...
                                // ✅ Сонгосон мөрүүдийг DalanJIlhunPrint рүү дамжуулах
...
export default DalanJilhunShiljuuleh;
const columns = [
    {
        name: "id",
        label: "№",
